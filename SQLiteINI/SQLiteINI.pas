...
type
  ...
  public
    ...
    procedure DeleteAll;
    {function EraseSection erases an entire section of an INI DB}
    function EraseSection(const SectionID: Integer): Integer;
    {procedure ReadSection reads all the keys from a specified section of an INI DB into a TList<TKeys>}
    procedure ReadSection(const SectionID: Integer; KeysList: TList<TKeys>);
    {procedure ReadSections Reads all the sections in an INI DB into a TList<TSections>}
...
function TmsaSQLiteINI.SectionExists(const SectionName: string): Integer;
begin
...
function TmsaSQLiteINI.SectionId(const SectionName: string): Integer;
begin
  Result := SectionExists(SectionName);
end;

constructor TmsaSQLiteINI.Create(const ADatabaseFileName: string);
begin
...
function TmsaSQLiteINI.CreateSection(const SectionName, Description: string): Integer;
begin
  Result := SectionExists(SectionName);
  if Result > -1 then
    Exit;
...
function TmsaSQLiteINI.DeleteSection(const SectionID: Integer; DeleteKeysToo: Boolean): Integer;
begin
  Result := 0;
...
destructor TmsaSQLiteINI.Destroy;
begin
...
function TmsaSQLiteINI.Last_Insert_Rowid(): Integer;
begin
...
function TmsaSQLiteINI.Changes(): Integer;
begin
...
function TmsaSQLiteINI.KeysCount(const SectionID: Integer): Integer;
begin
...
procedure TmsaSQLiteINI.CreateMetadata;
var
  SQL: string;
begin
  try
...
procedure TmsaSQLiteINI.DeleteAll;
begin
...
function TmsaSQLiteINI.EraseSection(const SectionID: Integer): Integer;
begin
  Result := 0;
  FQuery.SQL.Text := 'DELETE FROM keys WHERE sections_id = :sections_id';
  FQuery.ParamByName('sections_id').AsInteger := SectionID;
...
procedure TmsaSQLiteINI.ReadSection(const SectionID: Integer; KeysList: TList<TKeys>);
begin
...
procedure TmsaSQLiteINI.ReadSections(SectionsList: TList<TSections>);
begin
  if SectionsList = nil then
    Exit;

  FQuery.SQL.Text := 'SELECT id, section_name, description, created_at, modif_at FROM sections';
  FQuery.Open;
